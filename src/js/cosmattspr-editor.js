...
define(['text!../html/cosmattspr-editor.html', //Layout of the Editor
        'css!../css/cosmattspr-editor.css', //Custom CSS of the Editor
    ...
    var __constants = {
        ...
        TEMPLATES: {
            ...
            cosmattspr_EDITOR: cosmattsprTemplateRef
        }
    };
    ...
    */
     ...
     *          e.g. [
     *             "<a href='http://www.comprodls.com/m1.0/interaction/cosmattspr'>i1</a>", 
     *             "<a href='http://www.comprodls.com/m1.0/interaction/cosmattspr'>i2</a>"
     *              ]   
    ...
    function __parseAndUpdateJSONForRivets(){
        for(var i=0; i <__interactionIds.length; i++){
           var processedArray = [];
           __editedJsonContent.content.interactions[i].cosmattspr.forEach(function(obj, index){
                var processedObj = {};
            ...
            __editedJsonContent.content.interactions[i].cosmattspr = processedArray; 
        }
    }
    ...
    function __removeItem(event, element, interaction){
        __editedJsonContent.content.interactions[interaction].cosmattspr.splice(element.customAttribs.index,1);
        for(var option=element.index; option<__editedJsonContent.content.interactions[interaction].cosmattspr.length; option++){
            obj.interactions[interaction].cosmattspr[option].customAttribs.index--;
        }
    ...
    function __addItem(event, content, interaction){
        ...
        newObj.customAttribs.index = content.interactions[interaction].cosmattspr.length;
        content.interactions[interaction].cosmattspr.push(newObj);
        __state.hasUnsavedChanges = true;
        activityAdaptor.itemChangedInEditor(__transformJSONtoOriginialForm(), uniqueId);
    }
    ...
    function __bindSortable(){
        $(".sortable").sortable({
            ...
            stop: function( event, ui ) {
                ...
                var removedItem = __editedJsonContent.content.interactions[interactIndex].cosmattspr.splice(prevIndex, 1);
                __editedJsonContent.content.interactions[interactIndex].cosmattspr.splice(currentIndex,0,removedItem[0]);
                
                /* Update index property of customAttribs for each element*/
                $.each(__editedJsonContent.content.interactions[interactIndex].cosmattspr, function(index, value){
                    __editedJsonContent.content.interactions[interactIndex].cosmattspr[index].customAttribs.index = index;
                });
    ...
    function __handleRadioButtonClick(event){
        ...
        __editedJsonContent.content.interactions[interactionIndex].cosmattspr.forEach(function(obj, index){
            if(__editedJsonContent.content.interactions[interactionIndex].cosmattspr[index].customAttribs.key ==  $(currentTarget).attr('key')){
                __editedJsonContent.content.interactions[interactionIndex].cosmattspr[index].customAttribs.isCorrect = true;
            } else{
                __editedJsonContent.content.interactions[interactionIndex].cosmattspr[index].customAttribs.isCorrect = false;
            }
        });
    ...
    function __transformJSONtoOriginialForm(){
        ...
        for(var interaction=0;interaction <__finalJSONContent.content.interactions.length; interaction++){
            ...
            for(var option=0;option<content.cosmattspr.length;option++){
                content.cosmattspr[option][content.cosmattspr[option].customAttribs.key] = content.cosmattspr[option].customAttribs.value;
                delete content.cosmattspr[option].customAttribs;
            }
